...
public class Pointer : MonoBehaviour {
	...
	void Update () {
		...
		if(selecting){
			...
			/*
			if(selectedObj != null){
				selector.position = selectedObj.position;
				selector.rotation = selectedObj.rotation;
			}
			*/
	...
	public void setPointer(PointerStatus s, Vector3 target){
		...
		ptr.position = target;
		ptr.rotation = Quaternion.FromToRotation (Vector3.up, target);
		showing = true;
		if (!modeSet) {
			status = s;
	...
	public void setPointer(PointerStatus s, Vector3 target, Vector3 relativeTo){
		ptrRenderer.enabled = true;
		ptr.position = target;
